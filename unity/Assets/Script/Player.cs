...
public class Player : MonoBehaviour {
	...
	void DoIdel(){
		...
		m_AIData.targetPoint = CheckEnemy (out fDist); //尋找目標
	...
	void DoWander(){
		float fDist = 10000.0f;
		m_AIData.targetPoint = CheckEnemy (out fDist);
		if (m_AIData.targetPoint != null) {
	...
	//尋找目標
	GameObject CheckEnemy(out float fOutMinDist){ //(裡面回傳距離，判定距離過小時直接進行攻擊等等)

		GameObject [] gos = SceneManager.m_Instance.m_Target;
		Vector3 tVec;
		float fDist = 0.0f;
