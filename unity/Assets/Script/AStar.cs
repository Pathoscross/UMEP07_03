...
public class AStar : MonoBehaviour {
	...
	public bool GetAStarSeekPoint(ref int currentIndex,ref Vector3 rPos){ //目前所在的index,
		int iLength = m_PathList.Count;
		Vector3 vPos;
		//從終點抓回去
		for(int i = iLength - 1; i >= currentIndex; i--) {
			vPos = (Vector3)m_PathList[i];
			bool bCol = Physics.Linecast(this.transform.position, vPos, 1 << LayerMask.NameToLayer("Obstacle"));
